...
nb_file = 50
delta_f = 5000/4096
lg_file = len(np.loadtxt("Projet 6/PROJET6.2/kundt1/kundt1.txt")[:, 0])
mod     = []
phas    = []
...
for k_file in range(1, nb_file + 1):
    data        = np.loadtxt("Projet 6/PROJET6.2/kundt1/kundt{}.txt".format(k_file))
    if k_file != 1:
...
w   = 2*np.pi*f
k   = w/c_0
phi = 0.1             #décale la courbe
R   =  1       #augmente l'amplitude
p_th = []
for i in range(nb_file):
    p_th.append(np.abs(np.exp(1j* k* x[i]) + R* np.exp(1j* phi)* R* np.exp(-1j* k* x[i])))
...
ax[0].set_title("Pression pour le matériau kundt1 à la fréquence {:.2f} Hz".format(fq[indice]))
