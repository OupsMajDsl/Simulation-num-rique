...
nb_file = 50
delta_f = 5000/4096
lg_file = len(np.loadtxt("Projet 6/PROJET6.2/kundt3/kundt1.txt")[:, 0])
mod     = []
phas    = []
...
for k_file in range(1, nb_file + 1):
    data        = np.loadtxt("Projet 6/PROJET6.2/kundt3/kundt{}.txt".format(k_file))
    if k_file != 1:
...
w   = 2*np.pi*f
k   = w/c_0
phi = 1.1            #décale la courbe
R   = 0.35        #augmente l'amplitude
p_th = []
...
ax[0].set_title("Pression pour le matériau kundt3 à la fréquence {:.2f} Hz".format(fq[indice]))
